...
export function useExpenseCategoryMutations() {
  ...
  const updateExpenseCategory = useApiMutation({
    mutationFn: ({ id, data }: { id: number; data: ExpenseCategoryPayload }) =>
  ...
  const deleteExpenseCategory = useApiMutation({
    ...
    successMessage: "Expense category archived successfully.",
    invalidateQueries: [
      ...sharedInvalidations,
      { queryKey: ["expense-categories-archived"] },
    ],
  })

  const activateExpenseCategory = useApiMutation({
    mutationFn: (id: number) => api.patch(`${url}${id}/`, { is_active: true }),
  ...
  const deactivateExpenseCategory = useApiMutation({
    mutationFn: (id: number) => api.patch(`${url}${id}/`, { is_active: false }),
