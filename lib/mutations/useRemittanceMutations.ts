...
export function useRemittanceMutations() {
  ...
  const recalculateRemittance = useApiMutation({
    mutationFn: (id: number) => api.post(`${url}${id}/recalculate/`),
    successMessage: "Remittance recalculated with latest sales and expenses.",
    invalidateQueries: [
      { queryKey: ["remittances"] },
      ...analyticsKeys.map((key) => ({ queryKey: key })),
    ],
