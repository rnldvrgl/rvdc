		...
		onSuccess: async () => {
			...
			if (payrollId) {
				try {
					...
					toast.error(
						"Earning added but failed to recalculate payroll. Please click Recompute manually.",
					);
				}
			}
		},
		...
		onSuccess: async () => {
			...
			if (payrollId) {
				try {
					...
					console.error("Failed to recompute payroll:", error);
					toast.error(
						"Earning deleted but failed to recalculate payroll. Please click Recompute manually.",
					);
				}
			}
		},
