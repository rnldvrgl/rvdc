		...
		onSuccess: async () => {
			...
			if (payrollId) {
				try {
					...
					toast.error(
						"Deduction added but failed to recalculate payroll. Please click Recompute manually.",
					);
				}
			}
		},
		...
		onSuccess: async () => {
			...
			if (payrollId) {
				try {
					...
					console.error("Failed to recompute payroll:", error);
					toast.error(
						"Deduction deleted but failed to recalculate payroll. Please click Recompute manually.",
					);
				}
			}
		},
...
export const useActivateManualDeduction = () => {
	...
	return useMutation({
		...
		onError: (error: unknown) => {
			...
			toast.error(
				err.response?.data?.detail || "Failed to activate deduction",
			);
		},
	});
};
