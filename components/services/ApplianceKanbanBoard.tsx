...
function DroppableColumn({
    ...
    return (
        <div
            ref={setNodeRef}
            className={`flex flex-col rounded-lg border bg-muted/30 p-2 min-h-[120px] transition-all duration-200 ${isOver && isValidTarget
...
function ApplianceCard({ appliance }: { appliance: ServiceAppliance }) {
    ...
    return (
        <motion.div
            ...
            {...listeners}
            layout
            className={`rounded-md border bg-background p-2 cursor-grab active:cursor-grabbing transition-shadow ${isDragging ? "opacity-40 shadow-lg" : "shadow-sm hover:shadow"
                }`}
        >
