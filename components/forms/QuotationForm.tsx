    ...
    return (
        <div className="space-y-1.5">
            ...
            <Button
                ...
                size="sm"
                onClick={addLine}
                className="mt-1 h-7 border-dashed text-xs text-primary border-primary/30 hover:bg-primary/5"
            >
...
export default function QuotationForm({
    ...
    const [clientSignature] = useState(
    ...
    const [clientAcceptanceName] = useState(
        quotation?.client_acceptance_name ?? "",
    )
    const [clientAcceptanceDate] = useState(
        quotation?.client_acceptance_date ?? "",
    )
    const [selectedRepId, setSelectedRepId] = useState<number | null>(null)
    const selectedRepresentative = useMemo(
        () => employees.find((employee) => employee.id === selectedRepId) ?? null,
        [employees, selectedRepId],
    )
    const selectedRepresentativeSignature =
        selectedRepresentative?.e_signature?.trim() ?? ""
    ...
    const addItem = useCallback(() => {
        setItems((prev) => [
            ...prev,
            { id: generateId(), airconUnitId: null, discount: 0, airconModelId: null, description: "", qty: 1, price: 0, promoPrice: null },
        ])
    }, [])
    ...
    const itemDiscountTotal = items.reduce(
        (sum, item) => sum + item.qty * (item.discount || 0),
        0,
    )
    ...
    return (
        <div className="space-y-6 pb-6 sm:space-y-8 sm:pb-8">
            ...
            <section className="space-y-4 rounded-2xl border border-border/60 bg-card p-4 shadow-sm sm:p-5">
                ...
                <div className="space-y-3">
                    ...
                    <div className="rounded-lg border border-border/60 space-y-1 bg-muted/30 px-3 py-2 text-xs text-muted-foreground">
                        <div className="font-medium text-foreground mb-1.5">Quotation Header</div>
            ...
            <section>
                ...
                <Tooltip>
                    ...
                    <TooltipContent>Add a new line item to the quotation</TooltipContent>
                </Tooltip>
                            ...
                            <div className="flex items-center justify-between gap-2">
                                <div className="space-y-1">
                                    <p className="text-xs font-medium text-foreground">
                                        {selectedRepresentative?.full_name ?? "Selected representative"}
                                    </p>
                                    <p className="text-[11px] text-muted-foreground">
                                        {selectedRepresentativeSignature
                                            ? "Signature on file. Upload or replace it below."
                                            : "No signature on file for the selected representative."}
                                    </p>
                                </div>
                                <Badge
                                    variant={selectedRepresentativeSignature ? "default" : "secondary"}
                                    className="shrink-0"
                                >
                                    {selectedRepresentativeSignature ? "Signature found" : "No signature"}
                                </Badge>
                            </div>
                            {selectedRepresentativeSignature ? (
                                <div className="overflow-hidden rounded-md border bg-muted/30">
                                    <Image
                                        src={selectedRepresentativeSignature}
                                ...
                                <div className="rounded-md border border-dashed bg-muted/20 px-3 py-4 text-xs text-muted-foreground">
                                    Select a representative with a saved signature or upload one below.
                                </div>
                            )}
                            <SignatureInput
                                label={
