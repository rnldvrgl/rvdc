...
import { useStockAudit } from "@/lib/queries/inventory/useStocks"
...
export default function StockAuditDialog({
  ...
  const { auditStallStock } = useStallStockMutations()
        ...
        {isLoading ? (
          ...
          <Form {...form}>
            ...
            >
                ...
                <CardContent className="px-6 space-y-2">
                  ...
                  <div className="grid grid-cols-3 gap-3">
                    ...
                    <BreakdownItem
                      label="Reserved"
                      value={
                        auditData?.reserved_quantity ?? stock.reserved_quantity
                      }
              ...
              {/* Active Reservations */}
              {auditData && auditData.reservations.length > 0 && (
                <Card>
                  <CardContent className="px-6 space-y-2">
                    <div className="flex items-center gap-1 font-semibold text-sm">
                      <TriangleAlert className="size-4 text-amber-500" />
                      Active Reservations ({auditData.reservations.length})
                    </div>
                    <div className="max-h-40 overflow-y-auto space-y-1.5">
                      {auditData.reservations.map((r, i) => (
                        <div
                          key={i}
                        ...
                        >
                          <div className="flex flex-col">
                            <span className="font-medium">
                              Service #{r.service_id} · {r.client_name}
                            </span>
                            <span className="text-muted-foreground capitalize">
                              {r.service_type.replace("_", " ")} ·{" "}
                              {r.service_status.replace("_", " ")}
                            </span>
                          </div>
                          <span className="font-mono font-medium">
                            {r.quantity_used} {auditData.item_unit}
                          </span>
                        </div>
                      ))}
                    </div>
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-muted-foreground">
                          {hasDiscrepancy
                            ? discrepancy > 0
                              ? "Physical count is higher than system — stock will be increased."
                              : "Physical count is lower than system — stock will be decreased."
