...
/** Renders a KB value, auto-converting to MB above 1024 — animates the number. */
function AnimatedKbSize({ valueKb }: { valueKb: number }) {
    const isMb = valueKb > 1024
    const displayValue = isMb ? valueKb / 1024 : valueKb
    return (
        <>
            <AnimatedNumber
                value={displayValue}
                format={{ minimumFractionDigits: isMb ? 1 : 0, maximumFractionDigits: isMb ? 1 : 0 }}
            />{" "}
            {isMb ? "MB" : "KB"}
        </>
    )
...
/** Renders an MB value, auto-converting to GB above 1024 — animates the number. */
function AnimatedMbSize({ valueMb }: { valueMb: number }) {
    const isGb = valueMb > 1024
    const displayValue = isGb ? valueMb / 1024 : valueMb
    return (
        <>
            <AnimatedNumber
                value={displayValue}
                format={{ minimumFractionDigits: isGb ? 1 : 0, maximumFractionDigits: isGb ? 1 : 0 }}
            />{" "}
            {isGb ? "GB" : "MB"}
        </>
    )
...
export function ServerMaintenanceCard() {
    ...
    const getBarBgColor = (percent: number) => {
        if (percent >= 90) return "bg-destructive/10"
    ...
    const getStateColor = (state: string) => {
        ...
        if (state === "restarting") return "bg-warning"
        ...
        >
            <div className="flex items-center justify-between">
                <div className="flex items-center gap-2.5">
                    <div
                        className={`rounded-lg p-2 ${percent >= 90 ? "bg-destructive/15" : percent >= 75 ? "bg-warning/15" : "bg-success/15"}`}
                    >
                        <Icon className={`size-4 ${getUsageColor(percent)}`} />
                    </div>
                        ...
                        >
                            <TabsList className="group-data-[orientation=horizontal]/tabs:h-auto! mb-1 grid w-full grid-cols-2 items-stretch gap-1 p-1 max-[420px]:grid-cols-1 sm:grid-cols-5">
                                ...
                                <TabsTrigger value="actions">
                                    ...
                                    Actions
                                    {hasHighUsage && (
                                        <span className="ml-1.5 size-2 rounded-full bg-warning animate-pulse" />
                                    )}
                                </TabsTrigger>
                                <TabsTrigger
                                    value="backups"
                            ...
                            {/* ===== OVERVIEW TAB ===== */}
                            <TabsContent value="overview" className="mt-0" asChild>
                                            ...
                                            <div className="space-y-3">
                                                <div className="flex items-center justify-between">
                                                    <div className="flex items-center gap-2">
                                                        ...
                                                        >
                                                            <AnimatedMbSize valueMb={data.media_total_mb} />
                                                        </Badge>
                                                    </div>
                                                    <Button
                                                        variant="ghost"
                                                    ...
                                                    >
                                                        {showMediaFiles ? (
                                                            ...
                                                            <>
                                                                ...
                                                                <AnimatedNumber
                                                                    value={data.large_media_files.length}
                            ...
                            {/* ===== CONTAINERS TAB ===== */}
                            <TabsContent value="containers" className="mt-0" asChild>
                            ...
                            {/* ===== SCHEDULED JOBS TAB ===== */}
                            <TabsContent value="scheduled" className="mt-0" asChild>
                                                ...
                                                {data.cron_jobs.map((job) => (
                                                    ...
                                                    >
                                                        <div className="flex items-start justify-between gap-3">
                                                            <div className="flex items-start gap-3 min-w-0">
                                                                <div
                                                                    className={`rounded-lg p-2 mt-0.5 ${job.last_status === "success"
                                                                        ? "bg-success/10"
                                                                ...
                                                                >
                                                                    <Clock
                                                                        className={`size-3.5 ${job.last_status === "success"
                                                                            ? "text-success"
                                                                            : job.last_status === "error"
                                                                                ? "text-destructive"
                                                                ...
                                                                <div className="min-w-0 space-y-0.5">
                                                                    ...
                                                                    <div className="flex items-center gap-3 text-[10px] text-muted-foreground pt-0.5">
                                                                        ...
                                                                        {job.log_size_kb > 0 && (
                                                                            <span>
                                                                                Log: <AnimatedKbSize valueKb={job.log_size_kb} />
                                                                            </span>
                                                                        )}
                                                                    </div>
                                                                </div>
                                            ...
                                            <div className="grid gap-2">
                                                {data.management_commands.map((cmd) => {
                                                    ...
                                                    return (
                                                        <div
                                                            key={cmd.id}
                                                            className={`rounded-lg border p-3 transition-colors ${cmd.destructive
                                                        ...
                                                        >
                                                            <div className="flex items-start justify-between gap-3">
                                                                <div className="flex items-start gap-3 min-w-0">
                                                                    <div
                                                                        className={`rounded-lg p-2 mt-0.5 ${cmd.destructive
                                                                            ? "bg-destructive/10"
                                                                            : "bg-muted"
                                                                            }`}
                                                                    >
                                                                        <Wrench
                                                                            className={`size-3.5 ${cmd.destructive
                                                                                ? "text-destructive"
                                                                                : "text-muted-foreground"
                                                                                }`}
                                                                        />
                            ...
                            {/* ===== ACTIONS TAB ===== */}
                            <TabsContent value="actions" className="mt-0" asChild>
                                <motion.div {...fadeIn} className="space-y-4">
                                    ...
                                    <div className="grid gap-3">
                                        {CLEANUP_ACTIONS.map((action) => {
                                            ...
                                            return (
                                                <div
                                                    key={action.id}
                                                    className={`group relative rounded-xl border p-4 transition-colors ${action.destructive
                                                        ? "hover:border-destructive/40"
                                                        : "hover:border-foreground/20"
                                                        }`}
                                                >
                                                    <div className="flex items-start justify-between gap-4">
                                                        <div className="flex items-start gap-3">
                                                            <div
                                                                className={`rounded-lg p-2 ${action.destructive
                                                                    ? "bg-destructive/10"
                                                                    : "bg-muted"
                                                                    }`}
                                                            >
                                                                <Icon
                                                                    className={`size-4 ${action.destructive
                                                                        ? "text-destructive"
                                                                        : "text-muted-foreground"
                                                                        }`}
                                                                />
                            ...
                            {/* ===== BACKUPS TAB ===== */}
                            <TabsContent value="backups" className="mt-0" asChild>
                                <motion.div {...fadeIn} className="space-y-4">
                                    <div className="flex flex-col gap-3 md:flex-row sm:items-center md:justify-between flex-wrap">
                                        <div className="flex items-start gap-2 rounded-lg border bg-muted/30 p-3 flex-1 sm:mr-3">
                                            <Info className="size-4 text-muted-foreground shrink-0 mt-0.5" />
                                            <p className="text-xs text-muted-foreground leading-relaxed">
                                            ...
                                            <AnimatePresence initial={false}>
                                                {backups.map((backup) => (
                                                    <motion.div
                                                        ...
                                                        className="overflow-hidden"
                                                    >
                                                        <div className="group flex items-center justify-between rounded-xl border p-4 hover:border-foreground/20 transition-colors">
                                                            <div className="flex items-center gap-3">
            ...
            >
                <DialogContent className="max-w-3xl max-h-[80vh]">
                    ...
                    {logsDialog.loading ? (
                        ...
                        <pre className="bg-foreground text-background rounded-lg p-4 text-xs leading-relaxed overflow-auto max-h-[60vh] whitespace-pre-wrap break-all font-mono">
                            {logsDialog.logs}
                        </pre>
                    )}
                </DialogContent>
