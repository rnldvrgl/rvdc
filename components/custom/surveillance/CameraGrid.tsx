      ...
      <div className="flex items-center justify-between gap-3 flex-wrap">
        <div className="flex items-center gap-2">
          {go2rtcStatus !== undefined && (
            <Badge
              variant={go2rtcStatus.configured && go2rtcStatus.running ? "default" : "destructive"}
              className="gap-1"
            >
              {go2rtcStatus.configured && go2rtcStatus.running ? (
                <Wifi className="size-3" />
              ) : (
                <WifiOff className="size-3" />
              )}
              {go2rtcStatus.configured && go2rtcStatus.running ? "go2rtc online" : "go2rtc offline"}
            </Badge>
          )}
          {go2rtcStatus?.error && (
